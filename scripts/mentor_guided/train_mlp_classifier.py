...
def main():
    ...
    # The memory will be reused by model loading and training (PyTorch's cache allocator)
    if args.memory_lock > 0:
        ...
        if fill_bytes > 0:
            ...
            if is_main_process():
                ...
                logger.info(f"  Lock tensor size: {fill_bytes/1024**3:.2f} GB (will be reused during model loading and training)")
        else:
            if is_main_process():
