...
def load_all_subsets_data(base_dir: str, split: str = "train") -> Dict[int, List[Dict]]:
    ...
    # Auto-detect subsets from data directory
    subsets = detect_subsets(base_dir, split)
    if is_main_process():
        logger.info(f"Detected subsets: {subsets}")
...
def eval_cascade_on_val(
    ...
    for i in range(n_samples):
        ...
        if not decided:
            # Use last stage
            item = val_data[TOKEN_LEVELS[-1]][i]
            m_len = item.get('mentor_length', 0)
            i_len = item.get('intern_length', item.get('num_tokens', 0))
            cascade_mentor_lens.append(m_len)
            cascade_intern_lens.append(i_len)
...
def main():
    ...
    parser.add_argument("--train-subset", type=str, default=None,
                        help="Which subset(s) for training. 'all' merges all subsets. Auto-detected from data-dir.")
    parser.add_argument("--eval-subset", type=str, default=None,
                        help="Which subset(s) for evaluation. 'all' evaluates each subset separately. Auto-detected from data-dir.")
    ...
    if not args.eval_only and is_main_process():
        ...
        if best_state:
            ...
            # No best_state (e.g., skip_epoch_cascade), save last model as best
            thresholds = [args.fixed_threshold] * len(TOKEN_LEVELS) if args.fixed_threshold else [0.5] * len(TOKEN_LEVELS)
            torch.save({'classifier': classifier_state, 'thresholds': thresholds}, os.path.join(args.output_dir, "best_model.pt"))
            torch.save(classifier_state, os.path.join(args.output_dir, "classifier_head.pt"))
            logger.info(f"Last model saved as best (skip_epoch_cascade mode)")
